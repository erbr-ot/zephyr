...
uint8_t ll_read_iso_link_quality(uint16_t handle, uint32_t *tx_unacked_packets,
		...
		do {
			...
			if ((sdu.size >= ISO_TEST_PACKET_COUNTER_SIZE) &&
				...
				if (cis->framed) {
					 ...
					 */
					isoal_tx_unframed_get_next_playload_number(source_handle,
									&sdu,
									&next_payload_number);
