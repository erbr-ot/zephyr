...
static void lp_sca_ntf(struct ll_conn *conn, struct proc_ctx *ctx)
{
	...
	pdu->status = ctx->data.scau.error_code;
	pdu->sca = ctx->data.scau.sca;
...
static void lp_comm_complete(struct ll_conn *conn, struct proc_ctx *ctx, uint8_t evt, void *param)
{
	...
	case PROC_SCA_UPDATE:
		...
		case PDU_DATA_LLCTRL_TYPE_UNKNOWN_RSP:
			...
			ctx->data.scau.error_code = BT_HCI_ERR_UNSUPP_FEATURE_PARAM_VAL;
			/* Fall through to complete procedure */
		case PDU_DATA_LLCTRL_TYPE_CLOCK_ACCURACY_RSP:
#if defined(CONFIG_BT_PERIPHERAL)
			if (conn->lll.role == BT_HCI_ROLE_PERIPHERAL &&
			    !ctx->data.scau.error_code &&
			    conn->periph.sca != ctx->data.scau.sca) {
				conn->periph.sca = ctx->data.scau.sca;
