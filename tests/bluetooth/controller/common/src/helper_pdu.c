...
void helper_pdu_encode_cis_req(struct pdu_data *pdu, void *param)
{
	...
	pdu->llctrl.cis_req.framed	=  p->framed;
	pdu->llctrl.cis_req.c_max_sdu	=  p->c_max_sdu;
	pdu->llctrl.cis_req.p_max_sdu	=  p->p_max_sdu;
...
void helper_pdu_verify_cis_req(const char *file, uint32_t line, struct pdu_data *pdu, void *param)
{
	...
	zassert_equal(pdu->llctrl.cis_req.p_phy, p->p_phy,
		      "p_phy mismatch.\nCalled at %s:%d\n", file, line);
	zassert_equal(pdu->llctrl.cis_req.framed, p->framed,
		      "framed mismatch.\nCalled at %s:%d\n", file, line);
	zassert_equal(pdu->llctrl.cis_req.c_max_sdu, p->c_max_sdu,
		      "c_max_sdu mismatch.\nCalled at %s:%d\n", file, line);
	zassert_equal(pdu->llctrl.cis_req.p_max_sdu, p->p_max_sdu,
		      "p_max_sdu mismatch.\nCalled at %s:%d\n", file, line);
