				...
				uint8_t length, struct isoal_pdu_rx *pdu_meta)
{
	...
	zassert_true(pdu_payload_size <= TEST_RX_PDU_PAYLAOD_MAX, "pdu_payload_size (%d)", pdu_payload_size);
	...
	switch (role)
	{
		...
		case BT_CONN_ROLE_CENTRAL:
			if (framed) {
				...
				latency = stream_sync_delay - group_sync_delay - (((iso_interval / sdu_interval) - 1) * iso_interval);
			}
			break;
...
ZTEST(test_rx_basics, test_sink_create_destroy)
{
	...
	for (int role=0; role<=3; role++) {
		...
		for (int i=0; i<CONFIG_BT_CTLR_ISOAL_SINKS; i++) {
			...
			zassert_equal(isoal_global.sink_state[sink_hdl[i]].session.pdus_per_sdu, pdus_per_sdu,
				"%s pdus_per_sdu %d should be %d for: \n\tBN %d \n\tFT %d \n\tISO Interval %dus \n\tSDU Interval %dus \n\tStream Sync Delay %dus \n\tGroup Sync Delay %dus",
				(framed ? "Framed" : "Unframed"),
				isoal_global.sink_state[sink_hdl[i]].session.pdus_per_sdu,
				pdus_per_sdu,
				burst_number, flush_timeout, iso_interval, sdu_interval, stream_sync_delay, group_sync_delay);

			if (framed) {
				zassert_equal(isoal_global.sink_state[sink_hdl[i]].session.latency_framed, latency,
					"%s latency framed %d should be %d", ROLE_TO_STR(role),
					isoal_global.sink_state[sink_hdl[i]].session.latency_framed, latency);
			} else {
				zassert_equal(isoal_global.sink_state[sink_hdl[i]].session.latency_unframed, latency,
					"%s latency unframed %d should be %d", ROLE_TO_STR(role),
					isoal_global.sink_state[sink_hdl[i]].session.latency_unframed, latency);
			}
...
ZTEST(test_rx_basics, test_sink_disable)
{
	...
	create_unframed_pdu(PDU_BIS_LLID_COMPLETE_END, &testdata[testdata_indx], (testdata_size - testdata_indx),
...
ZTEST(test_rx_unframed, test_rx_unframed_dbl_pdu)
{
	...
	create_unframed_pdu(PDU_BIS_LLID_START_CONTINUE, &testdata[testdata_indx], (testdata_size - testdata_indx),
				payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID,
				&rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_unframed, test_rx_unframed_seq_err)
{
	...
	/* PDU 2 Not transfered to ISO-AL ------------------------------------*/
	payload_number++;
	pdu_timestamp += 200;
	testdata_indx = testdata_size;
	testdata_size += 10;
...
ZTEST(test_rx_unframed, test_rx_unframed_dbl_pdu_invalid_llid1)
{
	...
	create_unframed_pdu(PDU_BIS_LLID_FRAMED, &testdata[testdata_indx], (testdata_size - testdata_indx),
				payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID,
				&rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_single_pdu_single_sdu)
{
	...
	create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID, &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[0] = add_framed_pdu_single(&testdata[testdata_indx], (testdata_size - testdata_indx),
	...
	ztest_expect_value(sink_sdu_write_test, dbuf, &rx_sdu_frag_buf);
	ztest_expect_value(sink_sdu_write_test, pdu_payload, &rx_pdu_meta_buf.pdu[2+pdu_data_loc[0]]);
	ztest_expect_value(sink_sdu_write_test, consume_len, (testdata_size - testdata_indx));
	ztest_returns_value(sink_sdu_write_test, ISOAL_STATUS_OK);
...
ZTEST(test_rx_framed, test_rx_framed_trppl_pdu_single_sdu)
{
	...
	create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID, &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[0] = add_framed_pdu_start(&testdata[testdata_indx], (testdata_size - testdata_indx),
				sdu_timeoffset, &rx_pdu_meta_buf.pdu_meta);
	...
	create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID, &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[1] = add_framed_pdu_cont(&testdata[testdata_indx], (testdata_size - testdata_indx),
				&rx_pdu_meta_buf.pdu_meta);
	...
	ztest_expect_value(sink_sdu_write_test, dbuf, &rx_sdu_frag_buf);
	ztest_expect_value(sink_sdu_write_test, pdu_payload, &rx_pdu_meta_buf.pdu[2+pdu_data_loc[1]]);
	ztest_expect_value(sink_sdu_write_test, consume_len, (testdata_size - testdata_indx));
	ztest_returns_value(sink_sdu_write_test, ISOAL_STATUS_OK);
	...
	create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID, &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[2] = add_framed_pdu_end(&testdata[testdata_indx], (testdata_size - testdata_indx),
				&rx_pdu_meta_buf.pdu_meta);
	...
	ztest_expect_value(sink_sdu_write_test, dbuf, &rx_sdu_frag_buf);
	ztest_expect_value(sink_sdu_write_test, pdu_payload, &rx_pdu_meta_buf.pdu[2+pdu_data_loc[2]]);
	ztest_expect_value(sink_sdu_write_test, consume_len, (testdata_size - testdata_indx));
	ztest_returns_value(sink_sdu_write_test, ISOAL_STATUS_OK);
...
ZTEST(test_rx_framed, test_rx_framed_trppl_pdu_dbl_sdu)
{
	...
	create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID, &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[1] = add_framed_pdu_end(&testdata[testdata_indx], (testdata_size - testdata_indx),
				&rx_pdu_meta_buf.pdu_meta);
	...
	pdu_data_loc[2] = add_framed_pdu_start(&testdata[testdata_indx], (testdata_size - testdata_indx),
				sdu_timeoffset, &rx_pdu_meta_buf.pdu_meta);
	...
	create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID, &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[3] = add_framed_pdu_end(&testdata[testdata_indx], (testdata_size - testdata_indx),
				&rx_pdu_meta_buf.pdu_meta);
	...
	ztest_expect_value(sink_sdu_write_test, dbuf, &rx_sdu_frag_buf[1]);
	ztest_expect_value(sink_sdu_write_test, pdu_payload, &rx_pdu_meta_buf.pdu[2+pdu_data_loc[3]]);
	ztest_expect_value(sink_sdu_write_test, consume_len, (testdata_size - testdata_indx));
	ztest_returns_value(sink_sdu_write_test, ISOAL_STATUS_OK);
...
ZTEST(test_rx_framed, test_rx_framed_zero_length_sdu)
{
	...
	/* Zero length SDU */
	pdu_data_loc[2] = add_framed_pdu_single(&testdata[testdata_indx], (testdata_size - testdata_indx),
				sdu_timeoffset, &rx_pdu_meta_buf.pdu_meta);
	...
	pdu_data_loc[3] = add_framed_pdu_start(&testdata[testdata_indx], (testdata_size - testdata_indx),
				sdu_timeoffset, &rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_dbl_pdu_dbl_sdu_padding)
{
	...
	create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID, &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[1] = add_framed_pdu_single(&testdata[testdata_indx], (testdata_size - testdata_indx),
				sdu_timeoffset, &rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_dbl_pdu_dbl_sdu_pdu_err1)
{
	...
	/* PDU with errors */
	create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_ERRORS, &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[0] = add_framed_pdu_single(&testdata[testdata_indx], (testdata_size - testdata_indx),
				sdu_timeoffset, &rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_dbl_pdu_dbl_sdu_pdu_err2)
{
	...
	/* PDU with errors */
	create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_LOST_DATA, &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[0] = add_framed_pdu_single(&testdata[testdata_indx], (testdata_size - testdata_indx),
				sdu_timeoffset, &rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_trppl_pdu_single_sdu_pdu_err1)
{
	...
	create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID, &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[3] = add_framed_pdu_single(&testdata[testdata_indx], (testdata_size - testdata_indx),
				sdu_timeoffset, &rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_trppl_pdu_single_sdu_seq_err1)
{
	...
	/* PDU 2 -------------------------------------------------------------*/
	/* PDU not transfered to the ISO-AL */
	init_rx_pdu_buffer(&rx_pdu_meta_buf);
...
ZTEST(test_rx_framed, test_rx_framed_trppl_pdu_dbl_sdu_pdu_err1)
{
	...
	create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID, &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[4] = add_framed_pdu_single(&testdata[testdata_indx], (testdata_size - testdata_indx),
				sdu_timeoffset, &rx_pdu_meta_buf.pdu_meta);
	...
	ztest_expect_value(sink_sdu_write_test, dbuf, &rx_sdu_frag_buf);
	ztest_expect_value(sink_sdu_write_test, pdu_payload, &rx_pdu_meta_buf.pdu[2+pdu_data_loc[4]]);
	ztest_expect_value(sink_sdu_write_test, consume_len, (testdata_size - testdata_indx));
	ztest_returns_value(sink_sdu_write_test, ISOAL_STATUS_OK);
